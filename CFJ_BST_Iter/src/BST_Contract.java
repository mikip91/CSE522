...
class BST_Iterator {
	...
	@Ensures({"stack.peek().value == root.min() && (count>=0 ||hasNext())", "root.ordered()"})
	public BST_Iterator(Tree root) {
		stack_left_spine(root);
		//System.out.println("stack_left_spine1" + root);
	}
	...
	@Requires({"count>= 0 && hasNext()"})
	@Ensures({"value >=old(value)", "old(count) > 0 || (stack.isEmpty() || result <= (stack.peek().value))", "old(count) == 0 || old(count) == count + 1 "})
	public int next() {
		if (count == 0) {
			Tree node = stack.pop();
			value = node.value;
			...
			//System.out.println("stack_left_spine2" + node.right);
		}
		count--;
		//System.out.println("stackTOP" + stack.peek().value);
		return value;
	}

	@Ensures({"stack.isEmpty() || stack.peek().value>=value", "(node==null || old(node).min()==stack.peek().value)"})
